...
export default function NewScreening() {
  ...
  const handleFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) { setValidation("Please upload a PNG, JPG, or PDF file."); setFileName(""); return; }
    if (file.size > 8 * 1024 * 1024) { setValidation("The file is larger than 8 MB. Please choose a smaller document."); setFileName(""); return; }
    setValidation("");
  ...
  const submit = async () => {
    if (!selectedIdentityId) { setValidation("Select a synthetic identity or choose Use Demo Document before screening."); return; }
    setValidation("");
    setProcessing(true);
    try {
      const result = await screen.mutateAsync({ identityId: selectedIdentityId, manipulationCodes: selectedCodes });
