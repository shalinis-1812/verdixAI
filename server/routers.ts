...
export const appRouter = router({
  ...
  cases: router({
    ...
    screen: protectedProcedure.input(z.object({ identityId: z.number().int().positive().optional(), manipulationCodes: z.array(z.string()).default([]), fileName: z.string().optional(), mimeType: z.string().optional(), fileBase64: z.string().optional(), rowIndex: z.number().int().nonnegative().optional(), uploadKind: z.enum(["photo", "dataset"]).optional() })).mutation(({ input }) => input.fileBase64 && input.fileName ? input.uploadKind === "photo" ? createPhotoScreeningCase(input.fileName, input.mimeType ?? "", input.fileBase64) : createUploadedScreeningCase(input.fileName, input.mimeType ?? "", input.fileBase64, input.rowIndex ?? 0) : input.identityId ? createScreeningCase(input.identityId, input.manipulationCodes) : null),
